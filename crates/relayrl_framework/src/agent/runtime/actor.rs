...
use crate::agent::builder::ClientModes;
#[cfg(any(feature = "nats-transport", feature = "zmq-transport"))]
use crate::agent::agent::{ActorInferenceMode, AlgorithmInitArgs};
use crate::agent::runtime::control::coordinator::ClientNamespace;
#[cfg(any(feature = "nats-transport", feature = "zmq-transport"))]
use crate::agent::runtime::control::lifecycle_manager::SharedTransportAddresses;
use crate::agent::runtime::control::state_manager::{
    ActorUuid, NameTag, decode_argmax, decode_continuous_bytes,
};
#[cfg(any(feature = "nats-transport", feature = "zmq-transport"))]
use crate::agent::client::runtime::data::router::InferenceRequest;
...
> ActorEntity<B, D_IN, D_OUT> for Actor<B, D_IN, D_OUT>
{
    ...
    async fn handle_shutdown(&mut self, _msg: RoutedMessage) -> Result<(), ActorError> {
        ...
        self.client_namespace
            .remove_id(crate::agent::ACTOR_CONTEXT, self.actor_id)
            .map_err(ActorError::from)?;
...
mod unit_tests {
    ...
    use crate::agent::runtime::control::coordinator::CHANNEL_THROUGHPUT;
    ...
    async fn spawn_loop_exits_on_shutdown_message() {
        ...
        actor
            .client_namespace
            .add_id(crate::agent::ACTOR_CONTEXT, actor_id)
            .unwrap();
        let handle = tokio::spawn(async move { actor.spawn_loop().await });
