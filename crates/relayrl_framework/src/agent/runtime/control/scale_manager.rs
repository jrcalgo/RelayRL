...
use crate::agent::builder::{
    ActorDataMode, LocalTrajectoryFileParams, ClientModes, uses_local_file_writing, uses_trajectory_cache,
};
#[cfg(any(feature = "nats-transport", feature = "zmq-transport"))]
use crate::agent::{ActorInferenceMode, AlgorithmInitArgs, ModelMode};
...
use crate::agent::runtime::control::state_manager::StateManager;
use crate::agent::runtime::data::router::buffer::{
    TrajectoryBufferTrait, TrajectorySinkError,
};
#[cfg(any(feature = "nats-transport", feature = "zmq-transport"))]
use crate::agent::client::runtime::data::router::receiver::{
    ClientTransportModelReceiver, TransportReceiverError,
};
use crate::agent::runtime::data::router::router_dispatcher::RouterDispatcher;
use crate::agent::runtime::data::router::{
    RoutedMessage, buffer::ClientTrajectoryBuffer, filter::ClientCentralFilter,
};
#[cfg(any(feature = "nats-transport", feature = "zmq-transport"))]
use crate::agent::client::runtime::data::sinks::transport_sink::TransportError;
#[cfg(any(feature = "nats-transport", feature = "zmq-transport"))]
...
impl<B: Backend + BackendMatcher<Backend = B>> ScaleManager<B> {
    ...
    pub(crate) async fn new(
        ...
        let scaling_id: ScaleManagerUuid = client_namespace
            .reserve_id_with(crate::agent::SCALE_MANAGER_CONTEXT, 67, 100)
    ...
    pub(crate) async fn send_client_ids_to_server(
            ...
            (&self.scaling_dispatcher, &self.shared_transport_addresses)
        {
            let scaling_entry = (
                ...
                crate::agent::SCALE_MANAGER_CONTEXT.to_string(),
                self.scaling_id,
            );
            scaling_dispatcher
                .send_client_ids(
    ...
    async fn start_transport_receiver(&mut self) -> Result<(), ScaleManagerError> {
        ...
        match (&self.training_dispatcher, &self.shared_transport_addresses) {
            (Some(training_dispatcher), Some(transport_addresses)) => {
                let _ = self
                    .client_namespace
                    .reserve_id_with(crate::agent::RECEIVER_CONTEXT, 1, 100)
                    .map_err(ScaleManagerError::from)?;
    ...
    pub(crate) async fn scale_routers_out(
        ...
        for _ in 0..router_add {
            ...
            let router_ns_str = format!(
                ...
                crate::agent::ROUTER_NAMESPACE_PREFIX,
                counter
            );
