...
use crate::agent::HyperparameterArgs;
use crate::agent::client::agent::{AlgorithmInitArgs, ModelMode};
use crate::agent::client::runtime::control::coordinator::ClientNamespace;
use crate::agent::client::runtime::control::lifecycle_manager::{
    SharedTransportAddresses, SharedZmqInferenceAddresses, SharedZmqTrainingAddresses,
};
use crate::agent::client::runtime::control::scale_manager::ScalingOperation;
...
use crate::agent::client::runtime::data::sinks::transport_sink::zmq::{
...
impl ZmqPool {
    ...
    fn create_dealer_socket(
        ...
        let identity: SocketUuid = self
            .client_namespace
            .reserve_id_with(crate::agent::ZMQ_CLIENT_CONTEXT, 117, 100)
    ...
    fn create_push_socket(
        ...
        let identity: SocketUuid = self
            .client_namespace
            .reserve_id_with(crate::agent::ZMQ_CLIENT_CONTEXT, 67, 100)
            .map_err(ZmqPoolError::from)?;
        socket.set_identity(identity.as_bytes())?;
    ...
    fn create_sub_socket(
        ...
        let identity: SocketUuid = self
            .client_namespace
            .reserve_id_with(crate::agent::ZMQ_CLIENT_CONTEXT, 69, 100)
            .map_err(ZmqPoolError::from)?;
        socket.set_identity(identity.as_bytes())?;
    ...
    pub(super) fn shutdown(&self) -> Result<(), TransportError> {
        ...
        if let Some(sockets) = &pool_guard.cached_sockets.inference_dealer_socket {
            for entry in sockets.iter() {
                ...
                pool_guard
                    .client_namespace
                    .remove_id(crate::agent::ZMQ_CLIENT_CONTEXT, socket_id)
