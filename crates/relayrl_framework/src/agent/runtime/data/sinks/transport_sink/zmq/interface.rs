use crate::agent::client::agent::ClientModes;
...
use crate::agent::client::runtime::data::sinks::transport_sink::combine_scaling_results;
...
impl<B: Backend + BackendMatcher<Backend = B>> SyncClientTransportInterface<B> for ZmqInterface<B> {
    fn new(
        ...
        let transport_id: TransportUuid = client_namespace
            .reserve_id_with(crate::agent::ZMQ_CLIENT_CONTEXT, 42, 100)
            .map_err(TransportError::from)?;

        let transport_entry = (
            client_namespace.to_string(),
            crate::agent::ZMQ_CLIENT_CONTEXT.to_string(),
            transport_id,
        );
