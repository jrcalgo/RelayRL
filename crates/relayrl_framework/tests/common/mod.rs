...
/// Identity ONNX graph with a dynamic batch axis: shape `[batch, 2]`, `f32` in and out.
/// Required by environment-driven rollouts, which feed `[n_envs, obs_dim]` tensors.
pub const TEST_ONNX_IDENTITY_BATCHED: &[u8] = &[
    ,
];
...
/// Loads [`TEST_ONNX_IDENTITY_BATCHED`] for environment-driven tests that feed `[n_envs, obs_dim]`.
pub fn load_batched_test_model_module() -> Result<(TempDir, ModelModule<TestBackend>), ModelError> {
    let model_dir = tempdir().expect("tempdir should be created");
    let metadata = ModelMetadata {
        model_file: "test.onnx".to_string(),
        model_type: ModelFileType::Onnx,
        input_dtype: DType::NdArray(NdArrayDType::F32),
        output_dtype: DType::NdArray(NdArrayDType::F32),
        // Concrete batch=1 used only for validation; the graph's first dim is dynamic.
        input_shape: vec![1, 2],
    ...
    let model_module =
        ModelModule::<TestBackend>::from_onnx_bytes(TEST_ONNX_IDENTITY_BATCHED.to_vec(), metadata)?;
