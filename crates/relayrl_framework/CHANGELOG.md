...
- **`ClientCoordinator::shutdown()` partial-teardown-on-error** - Capturing the match result did not fully fix `shutdown()`: the `Some(params)` arm still used `?` on `shutdown_all_actors()`, `send_shutdown_signal_to_server()`, `shutdown_transport()`, and both `clear_runtime_components()` calls, so any of those failing returned early from the whole function and skipped `runtime_params.take()`, leaving the coordinator advertised as live (`Some(runtime_params)`) over an already half-torn-down runtime. `shutdown()` now takes `runtime_params` out of the coordinator up front, clears the now-stale `inference_path_params`, and runs every teardown step best-effort — recording each step's failure instead of propagating it — so `runtime_params` always ends up `None` regardless of how many steps fail. A failed shutdown now returns `CoordinatorError::ShutdownPartialFailure { failures, drained_cache }`, which still carries whatever trajectory snapshot was already drained rather than discarding it, and actor selection for that drain falls back to a pre-shutdown actor snapshot if `shutdown_all_actors()` itself fails.
