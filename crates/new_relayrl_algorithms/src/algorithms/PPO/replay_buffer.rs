...
impl PPOReplayBuffer {
    ...
    pub fn get_obs_for_gae_blocking(&self) -> (Vec<TensorData>, usize) {
        ...
        (buffers.obs[..ptr * obs_dim].to_vec(), obs_dim)
    }
    ...
    pub fn get_obs_for_first_n_episodes(&self, n: usize) -> (Vec<TensorData>, usize) {
        ...
        (buffers.obs[..cut_step * obs_dim].to_vec(), obs_dim)
    }
    ...
    pub fn finalize_and_drain_blocking(&self, values: Vec<f32>) -> Option<PPOBatch> {
        ...
        let obs_end = (capacity * obs_dim).min(buffers.obs.len());
        let obs = buffers.obs[..obs_end].to_vec();
    ...
    pub fn finalize_and_drain_first_n_blocking(
        ...
        for (i, &(start, end, _)) in boundaries_n.iter().enumerate() {
            ...
            fresh_obs.extend_from_slice(&buffers.obs[obs_start..obs_end]);
        ...
        buffers.obs.copy_within(obs_end_cut..obs_total, 0);
        buffers.obs.truncate(remaining * obs_dim);
        buffers.act.copy_within(cut_step..total_steps, 0);
